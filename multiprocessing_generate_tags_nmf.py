...
from tailored_functions import generate_tags_nmf, generate_tags_in_list_nmf, find_close_neighbors
...
nmf_word_sim_matrix = execution_params['nmf_word_sim_matrix']
nmf_tag_sim_matrix = execution_params['nmf_tag_sim_matrix']
...
QUANTILE_THRESHOLD = execution_params['THRESHOLD']
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(N_PROCS) as pool:
        ...
        TASKS = [(corpus, nmf_word_sim_matrix, nmf_tag_sim_matrix, existing_tags, NEIGHBORS, tag_list, QUANTILE_THRESHOLD) \
        for corpus,tag_list in zip(L_sub_lists_of_corpus,L_sub_lists_of_tags)]

        print("TASK ready")
        results = [pool.apply_async(generate_tags_in_list_nmf, t) for t in TASKS]
        print("results ready")
